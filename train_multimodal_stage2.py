...
def train(cfg, writer, logger):
    ...
    modal_num = 3
    model = CustomModel(cfg, writer, logger, use_pseudo_label, modal_num)
    ...
    model.objective_vectors_group = objective_vectors['objective_vectors_group']
    model.objective_vectors_num_group = objective_vectors['objective_vectors_num_group']
    print('ojective_vectors_group: {}'.format(model.objective_vectors_group))
    print('ojective_vectors_num_group: {}'.format(model.objective_vectors_num_group))
    logger.info('ojective_vectors_group: {}'.format(model.objective_vectors_group))
    logger.info('ojective_vectors_num_group: {}'.format(model.objective_vectors_num_group))
    """
    ...
    for epoch in range(epoches):
        ...
        if use_pseudo_label and model.iter > 0:
        ...
        for (target_image, target_label, target_img_name) in datasets.target_train_loader:
            ...
            #source_modal_ids = torch.Tensor(source_modal_ids).cuda()
            target_modal_ids = []
            ...
            if (i + 1) % cfg['training']['print_interval'] == 0:
                unchanged_cls_num = 0
                if use_pseudo_label:
                    fmt_str = "Epoches [{:d}/{:d}] Iter [{:d}/{:d}]  Loss: {:.4f}  Loss_L2Loss: {:.4f}  Loss_pseudo: {:.4f}  Time/Image: {:.4f}"
                else:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        nargs="?",
        type=str,
        default='configs/adversarial_adaptation_stage2.yml',
