...
from augmentations import get_composed_augmentations, get_composed_augmentations_multilabels
...
def create_unimodal_dataset(cfg, writer, logger):
    """Create a dataset given the option.
    ...
    Example:
        ...
        >>> dataset = create_dataset(opt)
    """
    data_loader = CustomUnimodalDatasetDataLoader(cfg, writer, logger)
    dataset = data_loader.load_data()
    return dataset

def create_unimodal_pseudo_dataset(cfg, writer, logger):
    """Create a dataset given the option.
    ...
    Example:
        ...
        >>> dataset = create_dataset(opt)
    """
    data_loader = CustomUnimodalPseudoDatasetDataLoader(cfg, writer, logger)
    dataset = data_loader.load_data()
    return dataset
...
class CustomDatasetDataLoader():
    ...
    def __init__(self, cfg, writer, logger, source_modal_num = 2):
        ...
        for _s_i in range(source_modal_num):
            _cfg_source = cfg['data']['source_{}'.format(_s_i)]
            _source_train = find_dataset_using_name(_cfg_source['name'])

            augmentations = cfg['training'].get('augmentations', None)
            data_aug = get_composed_augmentations(augmentations)
            _source_modal_train = _source_train(_cfg_source, writer, logger, augmentations=data_aug)
            self.source_train.append(_source_modal_train)
            logger.info("{} source dataset has been created".format(_source_modal_train.__class__.__name__))
            print("dataset {} for source was created".format(_source_modal_train.__class__.__name__))

            ## create train loader
            _source_train_loader = DataProvider(
                dataset=_source_modal_train,
                batch_size=_cfg_source['batch_size'],
                shuffle=_cfg_source['shuffle'],
                num_workers=int(cfg['data']['num_workers']),
                drop_last=True,
                pin_memory=True,
            )
            self.source_train_loader.append(_source_train_loader)
        ...
        # status == valid
        self.source_valid = []
        self.source_valid_loader = []
        for _s_i in range(source_modal_num):
            _cfg_source_valid = cfg['data']['source_valid_{}'.format(_s_i)]
            if _cfg_source_valid != None:
                _source_valid = find_dataset_using_name(_cfg_source_valid['name'])
                _source_modal_valid = _source_valid(_cfg_source_valid, writer, logger, augmentations=None)
                self.source_valid.append(_source_modal_valid)
                logger.info("{} source_valid dataset has been created".format(_source_modal_valid.__class__.__name__))
                print("dataset {} for source_valid was created".format(_source_modal_valid.__class__.__name__))

                _source_valid_loader = torch.utils.data.DataLoader(
                    _source_modal_valid,
                    batch_size=_cfg_source_valid['batch_size'],
                    shuffle=_cfg_source_valid['shuffle'],
                    num_workers=int(cfg['data']['num_workers']),
                    drop_last=True,
                    pin_memory=True,
                )
                self.source_valid_loader.append(_source_valid_loader)
...
class CustomUnimodalDatasetDataLoader():
    """Wrapper class of Dataset class that performs multi-threaded data loading"""
...
class CustomUnimodalPseudoDatasetDataLoader():
    """Wrapper class of Dataset class that performs multi-threaded data loading"""
    ...
    def __init__(self, cfg, writer, logger):
        ...
        augmentations = cfg['training'].get('augmentations', None)
        data_aug = get_composed_augmentations_multilabels(augmentations)
        target_train = find_dataset_using_name(cfg_target['name'])
        self.target_train = target_train(cfg_target, writer, logger, augmentations=augmentations) # augmentations
        #self.target_train = target_train(cfg_target, writer, logger, augmentations=data_aug) # augmentations
        logger.info("{} target dataset has been created".format(self.target_train.__class__.__name__))
        print("dataset {} for target was created".format(self.target_train.__class__.__name__))
