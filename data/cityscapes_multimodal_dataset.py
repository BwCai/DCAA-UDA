...
class Cityscapes_Multimodal_loader(BaseDataset):
    """cityscapesLoader
    ...
    def __init__(
        ...
        print("[INFO] dataset root:", self.root)
        ...
        self.annotations_base = os.path.join(
            self.root[0], "gtFine", self.split
        )
        for _root in self.root:
            if _root.endswith("Cityscapes"):
                _images_base = os.path.join(_root, "leftImg8bit", self.split)
                _files = recursive_glob(rootdir=_images_base, suffix=".png") #find all files from rootdir and subfolders with suffix = ".png"
            elif _root.endswith("Cityscapes_foggy"):
                _images_base = os.path.join(_root, "leftImg8bit_foggy", self.split)
                _files = recursive_glob(rootdir=_images_base, suffix="_beta_0.01.png") #find all files from rootdir and subfolders with suffix = ".png"
            elif _root.endswith("Cityscapes_rain"):
                _images_base = os.path.join(_root, "leftImg8bit_rain", self.split)
                _files = recursive_glob(rootdir=_images_base, suffix=".png")
            else:
                assert False, "data root: {} is unknown."
    ...
    def __getitem__(self, index):
        ...
        if img_path.split(os.sep)[1] == "Cityscapes":
            lbl_path = os.path.join(
                self.annotations_base,
                img_path.split(os.sep)[-2],
                os.path.basename(img_path)[:-15] + "gtFine_labelIds.png",
            )
        elif img_path.split(os.sep)[1] == "Cityscapes_foggy":
            lbl_path = os.path.join(
                self.annotations_base,
                img_path.split(os.sep)[-2],
                os.path.basename(img_path)[:-31] + "gtFine_labelIds.png",
            )
        elif img_path.split(os.sep)[1] == "Cityscapes_rain":
            _img_name = os.path.basename(img_path)
            _sub_idx = _img_name.find("leftImg8bit")
            lbl_path = os.path.join(
                self.annotations_base,
                img_path.split(os.sep)[-2],
                _img_name[:_sub_idx] + "gtFine_labelIds.png",
            )
        else:
            assert False, "dataset is unknown!"
    ...
    def transform(self, img, lbl):
        ...
        if not np.all(np.unique(lbl[lbl != self.ignore_index]) < self.n_classes) and not np.all(np.unique(lbl[lbl != self.ignore_index]) >= 0):   #todo: understanding the meaning 
